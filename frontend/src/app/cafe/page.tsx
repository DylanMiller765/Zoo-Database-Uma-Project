...
import { ShoppingCart, Sparkles, Coffee, UtensilsCrossed, Loader2, Check, Search } from 'lucide-react';
    ...
    <div className="min-h-[calc(100vh-6rem)] py-10">
      ...
      <section className="mt-8">
        <div className="flex items-center justify-between mb-6">
          <div>
            <h2 className="text-3xl font-bold text-gray-900 mb-2">Menu Highlights</h2>
            <p className="text-gray-600">Delicious food and beverages</p>
          </div>
          <div className="hidden md:flex items-center gap-2 px-4 py-2 bg-sea_green-50 rounded-full border border-sea_green-200">
            <Coffee className="h-4 w-4 text-sea_green-600" />
            <span className="text-sm font-medium text-sea_green-700">Fresh Daily</span>
          </div>
        </div>
        ...
        {!loading && !error && items.length === 0 && (
          <div className="rounded-xl bg-gray-50 border border-gray-200 p-12 text-center">
            ...
            <p className="text-sm text-gray-500 mt-2">Check back soon for new menu items!</p>
          </div>
        )}
        ...
        {!loading && !error && filteredItems.length > 0 && (
          <div className="mt-8 rounded-xl bg-gradient-to-r from-sea_green-50 to-dark_spring_green-50 border border-sea_green-200 p-6 text-center">
            <div className="flex items-center justify-center gap-2 mb-2">
              <UtensilsCrossed className="h-5 w-5 text-sea_green-600" />
              <p className="text-sm font-semibold text-dark_spring_green-800">
                Menu Subject to Change
              </p>
            </div>
            <p className="text-xs text-gray-600">
              Ask about seasonal specials and allergen information.
            </p>
          </div>
        )}
      </section>
